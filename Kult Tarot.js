...
let ext = seal.ext.find('KultTarotSystem');
if (!ext) {
    ext = seal.ext.new('KultTarotSystem', '巫洁鸾', '1.4.0');
    seal.ext.register(ext);
}

// === 图片配置 ===
// 请确保此路径与您本地实际路径一致
const IMAGE_DIR = "C\data\images\Kult";
const FILE_PREFIX = "纯占卜版_";
...
function getCardImage(id) {
    let num = id.toString().padStart(2, '0');
    return `[CQ:image,file=${IMAGE_DIR}\\${FILE_PREFIX}${num}.png]`;
}
